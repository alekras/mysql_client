...
mysql_test_() ->
  [ 
    { setup, 
      ...
      [
        {module, helper_common_eunit},
        { foreachx, 
          fun testing:do_setup/1, 
          fun testing:do_cleanup/2, 
          [
            {{plain, 'DB_create_Table_create'},    fun(_X, Y) -> {timeout, 30, fun() -> dbCreate(Y), tableCreate(Y), ?PASSED end} end},
            {{compress, 'DB_create_Table_create'}, fun(_X, Y) -> {timeout, 30, fun() -> dbCreate(Y), tableCreate(Y), ?PASSED end} end},
...
%            {{compress, tableSelect4}, fun tableSelect4/2},
         ...
         {module, cursor_eunit},
         {module, concurrent_connection_eunit}
      ]
    }
  ].
...
%  ?debug_Fmt("Connection record= ~p; ~n", [Conn_record]),
  ?assertMatch(#connection{}, Conn_record),
...
%  ?debug_Fmt("~p~n", [R2]),
  ?assertEqual("characters: char[25]", Val),
...
tableSelect4(_X, C) -> {timeout, 100, fun() ->
  Connection = datasource:get_connection(C),
...
%  ?debug_Fmt("Field = ~p; ~n", [Rr]),
  ...
  case _TR = connection:transaction(Connection, fun (C) -> Inc_2(C, " ") end) of
    #mysql_error{} -> 
%      ?debug_Fmt("     Exception 1 = ~p; ~n", [_TR]),
      ?assert(true);
    _ -> ?assert(false)
  end,
...
%  ?debug_Fmt("Field = ~p; ~n", [Re]),
  ...
  case _TR1 = connection:transaction(Connection, fun (C) -> Inc_2(C, ""), C = 1 end) of
    #mysql_error{}  -> 
%      ?debug_Fmt("     Exception 2 = ~p; ~n", [_TR1]),
      ?assert(true);
    _ -> ?assert(false)
  end,
